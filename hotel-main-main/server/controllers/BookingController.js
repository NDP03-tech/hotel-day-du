...
const getBookings = async (req, res) => {
    try {
        const bookings = await Booking.find()
            ...
            .select('room user customerList checkInDate checkOutDate totalAmount status'); // Thêm status vào select
...
const updateBooking = async (req, res) => {
    const { roomId, checkInDate, checkOutDate, customerList, totalAmount, status } = req.body;  // Thêm status vào destructuring
    if (!roomId || !checkInDate || !checkOutDate || !customerList || !totalAmount || status === undefined) {  // Kiểm tra status
        return res.status(400).json({
            success: false,
            message: 'All fields are required',
        });
    ...
    try {
        ...
        if (booking) {
            ...
            booking.status = status;  // Cập nhật trường status
        }

        const updatedBooking = await booking.save();

        if (updatedBooking.room !== roomId) {
            const BookingRoom = await Room.findById(updatedBooking.room); // Dùng `findById` thay vì `findIndex`
